...
$app->post('/api/AmazonSimpleDB/getAttributes', function ($request, $response, $args) {
    
    $checkRequest = $this->validation;
    $validateRes = $checkRequest->validate($request, ['apiKey','apiSecret','region','itemName','domainName']);
    ...
    if(!empty($post_data['args']['attributeName'])) {
        $body['AttributeName'] = $post_data['args']['attributeName'];
    }
    ...
    try {
        if(!empty($body)) {
            $res = $client->getAttributes(
                $body
            )->getAll();
        } else {
            $res = $client->getAttributes()->getAll();
        }
